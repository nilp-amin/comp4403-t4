...
public class Interpreter implements StatementVisitor, ExpTransform<Value> {
    ...
    public ErrorValue visitErrorExpNode(ExpNode.ErrorNode node) {
        ...
        errors.fatal("PL0 Internal error: attempt to evaluate ErrorExpNode", node.getLocation());
    ...
    public Value visitIdentifierNode(ExpNode.IdentifierNode node) {
         ...
         */
        errors.fatal("PL0 Internal error: attempt to evaluate IdentifierNode", node.getLocation());
        return null; // Never reached
    }
    ...
    public Value visitBinaryNode(ExpNode.BinaryNode node) {
        ...
        switch (node.getOp()) {
            ...
            case DIV_OP -> {
                ...
                if (right == 0) {
                    runtime("Division by zero", node.getRight().getLocation(), currentFrame);
                }
                result = left / right;
            }
    ...
    public Value visitDereferenceNode(ExpNode.DereferenceNode node) {
        ...
        if (result == null) {
            runtime("variable accessed before assignment", node.getLocation(), currentFrame);
            return null; // Never reached
        }
        endExec("Dereference");
        return result;
